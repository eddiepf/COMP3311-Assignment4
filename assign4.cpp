...
void showSuper(){
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	sprintf(query,"SELECT TA.student_ID AS \"Student ID\", TA.last_name AS \"Last Name\", TA.first_name AS \"First Name\", TA.phone AS \"Phone number\" FROM TA, supervise WHERE supervise.staff_ID=\'%i\' AND supervise.student_ID=TA.student_ID",staff_id);
	SQLExecDirectA(hstmt, (SQLCHAR *)query, SQL_NTS);
...
void showSuperGroup(){
	...
	// Add your code here
	sprintf(query,"SELECT prof.staff_ID AS \"Professor staff ID\",prof.last_name AS \"Last Name\" ,prof.first_name AS \"First Name\" ,LISTAGG(supervise.student_ID || ' ' || nvl(TA.last_name,' ')|| ' '||nvl(TA.first_name,' '),',') WITHIN GROUP(ORDER BY supervise.student_ID) AS \"Supervising Students\" FROM TA,supervise,prof WHERE supervise.staff_ID=prof.staff_ID AND supervise.student_ID=TA.student_ID GROUP BY prof.staff_ID,prof.last_name,prof.first_name");
	SQLExecDirectA(hstmt, (SQLCHAR *)query, SQL_NTS);
	printIntoRow(hstmt,15);
	system("pause");
	/*SQLAllocStmt(hdbc, &hstmt);
...
void showCourseTA(){
	...
	// Add your code here
	SQLAllocStmt(hdbc, &hstmt);
	//sprintf(query,"SELECT offering.course_id, course.course_name, TA.student_id, TA.last_name, TA.first_name, TA.phone FROM TA, offering, course WHERE TA.offering_no=offering.offering_no");
	sprintf(query,"SELECT prof_teach.course_ID AS \"Course ID\",course.course_name AS \"Course name\",prof_teach.offering_no,TA.student_ID AS \"TA student ID\",TA.last_name AS \"TA Last Name\", \
				TA.first_name AS \"TA First Name\",TA.phone AS \"TA phone no.\" FROM TA,offering,prof_teach,course where offering.YearSemester=\'%s\' AND prof_teach.staff_ID=\'%i\' \
				AND prof_teach.course_ID=TA.course_ID AND offering.offering_no=prof_teach.offering_no AND offering.offering_no=TA.offering_no AND course.course_ID=prof_teach.course_ID",currentSem,staff_id);
	SQLExecDirectA(hstmt, (SQLCHAR *)query, SQL_NTS);
	SQLBindCol(hstmt,1,SQL_C_CHAR,course_ID,20,&cbcourse_ID);
	SQLBindCol(hstmt,2,SQL_C_CHAR,course_name,20,&cbcourse_name);
	SQLBindCol(hstmt,3,SQL_INTEGER,&offering_no,10,&cboffering_no);
...
void showTAPref(){
	...
	// Add your code here
	//LISTAGG(supervise.student_ID || ' ' || nvl(TA.last_name,' ')|| ' '||nvl(TA.first_name,' '),',') WITHIN GROUP(ORDER BY supervise.student_ID)
	sprintf(query,"SELECT TA.student_ID AS \"TA student ID\",TA.last_name AS \"Last Name\",TA.first_name AS \"First Name\",listagg(pref_offering.course_ID || ' ' || nvl(to_char(pref_offering.offering_no),' '),', ') \
						    WITHIN GROUP(ORDER BY pref_offering.course_ID) AS \"Preference List\" \
				           FROM TA,pref_offering where TA.student_ID=pref_offering.student_ID GROUP BY TA.student_ID,TA.last_name,TA.first_name");
	SQLExecDirectA(hstmt, (SQLCHAR *)query, SQL_NTS);
	printIntoRow(hstmt,15);
